  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できない場合' do
      ...
      it 'last_name_kanaが全角カタカナ入力でなければ登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include('Last name kana カタカナ文字を使用してください')
      end

      it 'first_name_kanaが全角カタカナ入力でなければ登録できない' do
        @user.first_name_kana = 'あいうえお'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name kana カタカナ文字を使用してください')
      end
      ...
      it 'sexが未選択では出品できない' do
        @user.sex_id = 1
        @user.valid?
        expect(@user.errors.full_messages).to include("Sex 性別欄を選択してください")
      end

      it 'existenceが未選択では出品できない' do
        @user.existence_id = 1
        @user.valid?
        expect(@user.errors.full_messages).to include("Existence 食物アレルギーの有無を選択してください")
      end
    end
  end
end